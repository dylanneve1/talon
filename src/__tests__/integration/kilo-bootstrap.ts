/**
 * Functional-test bootstrap for the **kilo** (and, by the same wire API,
 * **kilo**) backend against an in-process fake HTTP server
 * (`stub-remote/fake-remote-server.ts`).
 *
 * The backend adopts any server already listening on `KILO_PORT` (probes
 * `/global/health` and reuses it), so we start the fake on a fixed port that
 * the importing test sets via `vi.hoisted` BEFORE the kilo module reads
 * `KILO_PORT` at eval time, then boot the real composition root.
 */
...
export interface EnsureKiloBootedArgs {
  frontend?: TalonConfig["frontend"];
  gatewayHandler?: FrontendActionHandler;
}

export async function ensureKiloBooted(
  args: EnsureKiloBootedArgs = {},
): Promise<void> {
  if (booted) return;
  ...
  if (frontend !== "terminal") {
    if (!gatewayHandler) {
      throw new Error(
        "ensureKiloBooted: non-terminal frontend needs a gatewayHandler",
      );
    }
    gateway = new Gateway();
  ...
  // The kilo module reads KILO_PORT at eval — the test must have set
  // it (vi.hoisted) before importing this module. Start the fake there.
  const fixedPort = Number(process.env.KILO_PORT ?? 0);
  fakeServer = await startFakeRemoteServer(fixedPort);

  const workspace = mkdtempSync(resolve(tmpdir(), "talon-kilo-stub-"));
  ...
  const config: TalonConfig = {
    ...
    plugins: [],
    botDisplayName: "Talon (kilo stub)",
...
export async function teardownKiloBootstrap(): Promise<void> {
  if (gateway) {
    void gateway.stop();
    gateway = null;
  }
...
export interface RunKiloTurnArgs {
  prompt: string;
  turn: RemoteTurn;
  chatId?: string;
  senderName?: string;
  ...
  bootstrap?: EnsureKiloBootedArgs;
}

export interface RunKiloTurnResult {
  text: string;
...
export async function runKiloTurn(
  args: RunKiloTurnArgs,
): Promise<RunKiloTurnResult> {
  await ensureKiloBooted(args.bootstrap);
  if (!fakeServer) throw new Error("fake remote server not started");

  const {
    prompt,
    turn,
    chatId = "kilo-chat-" + Math.random().toString(36).slice(2, 10),
    senderName = "TestUser",
    isGroup = false,
