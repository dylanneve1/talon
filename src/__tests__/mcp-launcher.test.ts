 ...
 * Launcher tests.
 *
 * Unit-level: wrapMcpServer + ensureLauncher file I/O.
...
// ── ensureLauncher + wrapMcpServer ─────────────────────────────────────────

describe("mcp-launcher", () => {
  beforeEach(async () => {
    const tmp = mkdtempSync(join(tmpdir(), "talon-launcher-"));
    process.env.HOME = tmp; // paths.ts reads homedir() → this
  });
  ...
  it("ensureLauncher writes the script to ~/.talon/mcp-launcher.mjs", async () => {
    const { ensureLauncher } = await freshLauncher();
    const path = ensureLauncher();

    expect(existsSync(path)).toBe(true);
    const content = readFileSync(path, "utf-8");
    expect(content).toContain("Auto-generated by Talon");
    expect(content).toContain("spawn(cmd, args");
    expect(content).toContain('process.stdin.on("end"');
  });

  it("ensureLauncher skips rewrite when content is already current", async () => {
    const { ensureLauncher, _resetLauncherCacheForTests } =
      await freshLauncher();
    const path = ensureLauncher();
    const firstMtime = statSync(path).mtimeMs;

    _resetLauncherCacheForTests();
  ...
  it("ensureLauncher rewrites when on-disk content is stale", async () => {
    const { ensureLauncher, _resetLauncherCacheForTests } =
      await freshLauncher();
    const path = ensureLauncher();
    ...
    ensureLauncher();
    expect(readFileSync(path, "utf-8")).toContain("Auto-generated by Talon");
  });
...
/**
 * Import the launcher module fresh with a scratch HOME so ensureLauncher
 * writes into a temp directory. Vitest caches modules, so we cache-bust
 * via vi.resetModules() in the enclosing beforeEach.
 */
