...
// ── Memory pre-retrieval (Phase B) ──────────────────────────────────────────

describe("weaver memory pre-retrieval", () => {
  ...
  function makeWeaver(opts: {
    retrieveMemory?: MemoryRetriever;
    resolveActiveModel?: ReturnType<typeof stubResolveActiveModel>;
  }) {
    const query = vi.fn(async (queryParams: QueryParams) => ({
      text: `echo:${queryParams.text}`,
      durationMs: 1,
      inputTokens: 0,
      outputTokens: 0,
      cacheRead: 0,
  ...
  it("calls the retriever for message turns and passes the result to the backend", async () => {
    const retrieveMemory = vi.fn(async () => sampleMemory);
    const { weaver, query } = makeWeaver({ retrieveMemory });
    ...
    expect(query.mock.calls[0]![0].retrievedMemory).toEqual(sampleMemory);
  });

  it("does not call the retriever for non-message sources", async () => {
    const retrieveMemory = vi.fn(async () => sampleMemory);
    const { weaver, query } = makeWeaver({ retrieveMemory });

    await weaver.runTurn(params({ chatId: "cron-chat", source: "cron" }));

    expect(retrieveMemory).not.toHaveBeenCalled();
    expect(query.mock.calls[0]![0].retrievedMemory).toBeUndefined();
  });

  it("does not call the retriever when the no-model guard refuses the turn", async () => {
    const retrieveMemory = vi.fn(async () => sampleMemory);
    const { weaver, query } = makeWeaver({
      retrieveMemory,
      resolveActiveModel: (async () => ({
        model: null,
        ref: null,
    ...
    const result = await weaver.runTurn(params({ chatId: "no-model" }));

    expect(result.text).toContain("No model selected");
    expect(retrieveMemory).not.toHaveBeenCalled();
    expect(query).not.toHaveBeenCalled();
  });

  it("fails closed: a throwing retriever still runs the turn without memory", async () => {
    ...
    const result = await weaver.runTurn(
      params({ chatId: "fail-closed", prompt: "still works" }),
    );

    expect(retrieveMemory).toHaveBeenCalledTimes(1);
    expect(result.text).toBe("echo:still works");
    expect(query).toHaveBeenCalledTimes(1);
    expect(query.mock.calls[0]![0].retrievedMemory).toBeUndefined();
  });

  it("absent retriever dep leaves the backend params without retrievedMemory", async () => {
    const { weaver, query } = makeWeaver({});

    await weaver.runTurn(params({ chatId: "plain" }));
