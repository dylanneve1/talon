...
const codexFactory: BackendFactory = {
  ...
  async init(config, ctx) {
    ...
    const backend: QueryBackend = {
      ...
      // Auth-aware default: ChatGPT-OAuth accounts can't run `gpt-5-codex`
      // (CLI returns the "not supported when using Codex with a ChatGPT
      // account" mismatch), so they should reset to `gpt-5.5` instead.
      getDefaultModel: () => {
