...
type FileMountOptions = {
  root: string;
  description: string;
  writable: boolean;
};

/** Absolute path for `rel` under `root`, or undefined when it would escape. */
function realPath(root: string, rel: string): string | undefined {
...
function statMount(
  ...
  const abs = realPath(root, rel);
  if (!abs) return vfsError("invalid-path");
  if (rel === "" && !existsSync(abs)) {
    return vfsOk({ path: "", name: "", kind: "dir", writable, osPath: abs });
  }
  return statAt(rel, abs, writable);
}
...
function listMount(
  ...
  const entries = readdirSync(abs, { withFileTypes: true })
    .map((entry) => {
      ...
      const child = statAt(childRel, resolve(abs, entry.name), writable);
...
function readMount(root: string, rel: string): VfsResult<string> {
  const abs = realPath(root, rel);
  if (!abs) return vfsError("invalid-path");
  let stats;
  try {
...
function writeMount(
  ...
  return statAt(rel, abs, true);
}

export function createFileMount(options: FileMountOptions): VfsMount {
  ...
  return {
    ...
    writable,
    osRoot: root,
    stat: (rel) => statMount(root, writable, rel),
    list: (rel) => listMount(root, writable, rel),
    read: (rel) => readMount(root, rel),
    ...(writable
      ? {
          write: (rel: string, content: string) =>
            writeMount(root, rel, content),
        }
      : {}),
  };
}
